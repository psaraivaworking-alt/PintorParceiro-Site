...
function showMessage(msg, isError = false) {
    ...
    if (messageBox) {
        ...
        setTimeout(() => {
            ...
            messageBox.classList.remove('error-message', 'success-message'); // Limpa as classes
        }, 5000); 
    } else {
        alert(msg); // Fallback
    }
...
// --- Lógica para o botão de ver senha (reintroduzido e chamado se o elemento existir) ---
...
if (registerForm) {
    ...
    setupPasswordToggle('password', 'togglePassword'); // Chama a função aqui
    ...
    registerForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            console.log("Usuário autenticado no cadastro:", user.email, "UID:", user.uid); 
            ...
            console.error("Erro completo da operacao Firebase:", error); 
            const errorCode = error.code;
            const errorMessage = error.message;

            console.log("Firebase Error Code (codigo do erro):", errorCode);   
            console.log("Firebase Error Message (mensagem do erro):", errorMessage); 
            ...
            if (errorCode === 'auth/email-already-in-use') {
                ...
                showMessage("Criação de conta por e-mail/senha não habilitada. Verifique o Firebase Authentication > Sign-in method.", true);
            }
            else {
...
if (loginForm) {
    setupPasswordToggle('password', 'togglePassword'); // Chama a função para o login também
